export class LeakyBucket {
  ...
  startIdleTimer() {
    ...
    if (this.currentCapacity >= this.capacity && this.queue.length === 0 && !this.timer) {
  ...
  cleanQueue() {
    ...
    if (index >= 0) {
      this.queue.splice(index).forEach((item) => {
        if (!item.isPause) {
          if (this.debug)
            console.log(`Rejecting item with a cost of ${item.cost} because an item was added in front of it!`);
          item.reject(
            new Error(
